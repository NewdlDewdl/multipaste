...
Sometimes another tool puts something on your clipboard and you want it
kept, not lost the moment you copy the next thing. `Multipaste
--pin-current` pins whatever is on the clipboard **right now**, straight
from the command line, so the value lands in your pinned rail and is one
`⌘⇧V` away later:

```sh
printf 'sk-live-…my-api-key' | pbcopy
Multipaste --pin-current        # ← now it's pinned, safe from the next copy
```

**How it works (and why it's safe).** The running Multipaste owns
`history.json` in memory and rewrites it on every copy, so a second
process editing that file behind its back would be clobbered. Instead
`--pin-current` is a hidden one-shot flag that posts a
`DistributedNotification` (`MultipasteIPC.pinCurrent`) and exits *without
starting a second instance*; the already-running app observes it and does
the pin through its real `HistoryStore`: the normal dedup, persist, and
picker-refresh path. Two guarantees fall out of routing through the real
store:

- **Password managers stay excluded.** The same
  `org.nspasteboard.org` Concealed / Transient / AutoGenerated markers
  that keep 1Password clips out of history also skip them here, so a
  concealed clip is never pinned.
- **It's a set, never a toggle.** Re-running `--pin-current` on an
  already-pinned item is a no-op, so an external tool can pin the same
  value twice without the second call silently *un*pinning it.
  (`HistoryStore.pin(contentHash:)` is unit-tested to never unpin.)

**The `clip-pin` helper.** [`scripts/clip-pin`](scripts/clip-pin) wraps
the whole "copy **and** pin" gesture and then verifies the pin actually
landed in `history.json` before reporting success:
...
Reach for it anywhere you'd otherwise `pbcopy` a value you don't want to
lose.
...
- **`MultipasteCore`** (library, pure Swift, no AppKit):
  ...
  `MultiPasteSeparator`, `PasteFlavor`, `PasteRouting`, `PasteSynthesis`,
...
- **`Multipaste`** (executable, AppKit-bound):
  ...
  `MarkdownAttributedString`, `MenuBarController`, `MultipasteIPC`,
  `PasteSmokeCheck`, `Paster`, `Permissions`, `PermissionMonitor`,
  `PickerWindow`, `ScreenshotWatcher`, `SettingsWindowController`,
  `SingleInstance`, `SnippetEngine`, `ThumbnailCache`, `UpdateService`,
  `WelcomeWindow`, `main.swift`. The one-shot `--pin-current` /
  `--paste-smoke` CLI flags in `main.swift` reach the running app through
  `MultipasteIPC`'s `DistributedNotification` channel.
...
| `HistoryStore` (--pin-current IPC) | 5 | (v2.4.2) `pin(contentHash:)`, the idempotent SET-pinned behind the `--pin-current` CLI/IPC: sets pinned by content hash, returns false when the hash is absent, **never toggles off** (`pinByContentHashNeverUnpins`, so a repeated external pin can't accidentally unpin), persists, and notifies observers |
...
| `Contribution`         | 6     | CONTRIBUTING.md exists, CLA contains perpetual/worldwide/royalty-free/irrevocable grant, relicensing-right clause explicitly mentions proprietary closed-source, PolyForm Strict context explained, PR template links to CLA + has confirmation checkboxes + calls out relicensing; **the PR template carries no hard-coded `N tests` count** (it had drifted to a stale literal while the suite grew, so the count now lives only in the README, checked against the live registry) |
...
Sources/
  MultipasteCore/      ← testable, pure Swift:
                          ...
                          PasteFlavor  PasteRouting  PasteSynthesis
                          PasteboardAugmenter  Preferences  ProcessTable
                          QuickPick  ReleaseNotesFormatter
  ...
  Multipaste/          ← AppKit / system:
                          ...
                          MenuBarController  MultipasteIPC  Paster
                          PasteSmokeCheck  Permissions  PermissionMonitor
                          PickerWindow  ScreenshotWatcher
                          SettingsWindowController  SingleInstance
...
Tests/MultipasteCoreTests/   ← 323 tests across 27 suites, e.g.:
  ...
  VersionConsistencyTests  DocConsistencyTests  ReadmePolishTests
  … (the coverage table above lists every suite)
...
scripts/
  ...
  clip-pin         # copy AND pin a value from the CLI (Multipaste --pin-current)
  pbtool.swift     # pasteboard oracle for headless release validation
  plaintext-paste-smoke-test.swift  # plain-text (⇧↩) write, mirror layer
  preview-update-dialog.swift       # visual preview of the update alert
  screenshot-smoke-test.swift       # screenshot→clipboard pipeline smoke
```
...
[bug-report template](https://github.com/NewdlDewdl/multipaste/issues/new/choose). For
...
killed the expansion. v2.4.1 retargeted ⌘1–9 quick-pick to the recent
(unpinned) rail, so ⌘1 is always the most recent thing you copied instead
of landing on a pin, with one QuickPick policy driving the picker badges,
the ⌘digit handler, and the menu key equivalents alike. v2.4.2 added
`Multipaste --pin-current`: a hidden one-shot CLI flag that pins whatever
